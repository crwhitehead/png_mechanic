...
std::vector<DeflatePacket> header_scan(const std::vector<uint8_t>& compressed_data){
    ...
    Bitstream* bitstream = new Bitstream(compressed_data);
    ...
    for(size_t i=0; i<compressed_data.size()*8; i++){
        ...
        if (test_packet.block_type == 2) {
            ...
            */
            ...
            test_packet.bitstream = bitstream;
            if(safe_end){
    ...
    //std::cout << "Look! " << bitstream->size() << std::endl;
    return packets;
}
...
void print_packet_details(const std::vector<DeflatePacket>& packets) {
    std::cout << std::setw(10) << "Start Pos"
              << std::setw(15) << "Header Start"
              << std::setw(15) << "Data Start"
              << std::setw(10) << "End Pos"
              << std::setw(7) << "Block"
              << std::setw(7) << "Last"
              << std::setw(7) << "Safe"
    ...
    for (const auto& packet : packets) {
        std::cout << std::setw(10) << packet.start_position
                  << std::setw(15) << packet.header_start
                  << std::setw(15) << packet.data_start
                  << std::setw(10) << packet.end_position
                  << std::setw(7) << packet.block_type
...
std::vector<LZToken> DeflatePacket::symbolic_inflate() {
    ...
    if (block_type == 0) {
        ...
        HuffmanTable code_length_table = load_code_lengths(bitstream, hclen);
        if(!verifyLengths(code_length_table.lengths)){
            throw std::runtime_error("Invalid code table lengths!");
        }
        HuffmanTable literal_table = load_literal_lengths(bitstream, &code_length_table, hlit);
        HuffmanTable distance_table = load_distance_lengths(bitstream,&code_length_table, hdist);
        while (true) {
        ...
        throw std::runtime_error("Error: Invalid block type");
    }
    puts("TOKEN SIZE!");
    printf("%lu\n", tokens.size());
    this->lz_blocks = tokens.size();
    return tokens;
}
